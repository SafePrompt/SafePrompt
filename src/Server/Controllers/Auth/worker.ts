...
const worker = {
    signup: async (req, res, next) => {
        try {
            ...
            const {
                ...
                password,
                key,
            }: { username: string; password: string; key: string } = req.body;
            ...
            const usernameExistsObj: dbResponse[] = await db
                .query(`SELECT * FROM "user" WHERE username = $1`, [username])
                .then((res) => res.rows);

            if (usernameExistsObj.length === 0) {
                const orgExists: dbResponse[] = await db
                    .query(
                        'SELECT * FROM "user" WHERE key = $1 AND admin = true',
                        [key]
                    )
                    .then((res) => res.rows);
                ...
                if (orgExists.length === 1) {
                    await db.query(
                        'INSERT INTO "user" (username, password, key, admin) VALUES ($1, $2, $3, $4)',
                        [username, hashPassword, key, false]
                    );
                    res.locals.key = key;
    ...
    login: async (req, res, next) => {
        try {
            ...
            const queryResponse: QueryResult = await db.query(
            ...
            if (
                responseArr.length === 1 &&
                bcrypt.compareSync(password, responseArr[0].password) &&
                !responseArr[0].admin
            ) {
