    ...
    try {
        ...
        if (failed.length > 0) {
            ...
            if (res.locals.object.phone) {
                ...
                const phoneNumbers: string[] = res.locals.object.phone.filter(
                    (phone: string) => !phoneSyntax.test(phone)
                );
                ...
                for (let i: number = 0; i < phoneNumbers.length; i++) {
                    for (
                        ...
                        if (phoneNumbers[i] === failed[index].slice(1)) {
                            res.locals.object.phone =
                                res.locals.object.phone.filter(
                                    (numb: string) =>
                                        numb !== failed[index].slice(1)
                                );
                        }
                    }
                }
